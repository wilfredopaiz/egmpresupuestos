# 002 - Modelo de datos y migración SQL

## Objetivo

Crear todas las tablas en Supabase ejecutando la migración SQL manualmente desde el SQL Editor del dashboard.
...
profiles (1:1 con auth.users)
    ...
    └──► projects
            │
            ▼
         project_items ──► item_templates
```

### Decisiones de diseño

- `profiles` se crea automáticamente al registrarse un usuario (trigger)
- `sections` e `item_templates` tienen `user_id` nullable: si es NULL son datos del catálogo global, si tiene valor son datos personalizados del usuario
- `project_items` referencia a `item_templates` y a `projects`
- Se usa `uuid` para todas las PKs
- `project_status` es un enum de PostgreSQL
- Timestamps con `timestamptz` y defaults automáticos
...
```sql
-- ============================================
-- EGM Presupuestos - Migración completa
-- ============================================
...
-- user_id NULL = catálogo global, user_id NOT NULL = sección personalizada del usuario
...
-- user_id NULL = catálogo global, user_id NOT NULL = plantilla personalizada
...
CREATE TABLE public.projects (
  ...
  client_name text, -- nombre del cliente desnormalizado para cuando no hay client_id
...
Después de ejecutar la migración, verificar en **Table Editor** que existen:
...
Y en **Database → Types** que existe el enum `project_status`.

## Generar tipos TypeScript

Después de crear las tablas, generar los tipos:

```bash
npx supabase gen types typescript --project-id TU_PROJECT_ID > src/types/database.types.ts
```

O desde el dashboard: **API Docs → Introduction → Generating Types**.

## Archivos modificados

- `src/types/database.types.ts` (actualizar con tipos generados)
